...
def distill_train(train_loader, model, epoch, args, arch_network_type, teacher_lst, test_loader):
    objs = utils.AverageMeter()
    n_diff_pairs_meter = utils.AverageMeter()
    model.train()
    
    for step, ((archs, ranks), (test_archs, _)) in enumerate(zip(train_loader, test_loader)):
        archs = np.array(archs)
        ranks = np.array(ranks)
        n = len(archs)

        test_archs = np.array(test_archs)
        ...
        test_archs_1 = np.random.choice(test_archs, size = len(archs_1))
        test_archs_2 = np.random.choice(test_archs, size = len(archs_1))
        
        teacher_better_lst = []
        for teacher in teacher_lst:
            teacher_better = teacher.compare(test_archs_1, test_archs_2) > 0.5
            teacher_better_lst.append(teacher_better)

        teacher_better_lst = (sum(teacher_better_lst) >= len(teacher_lst) // 2 + 1).detach().cpu().numpy()

        teacher_pair_loss = model.cal_compare_loss(test_archs_1, test_archs_2, teacher_better_lst)
        pair_loss = model.cal_compare_loss(archs_1, archs_2, better_lst)
        loss = model.update_step(0.5 * teacher_pair_loss + pair_loss)
...
def main(argv):
    ...
    arch_network_cfg = cfg.pop("arch_network_cfg")

    model = model_cls(search_space, **arch_network_cfg)
    ...
    for i in range(args.split_num):
        teacher = model_cls(search_space, **arch_network_cfg)
        teacher.load(os.path.join(args.teacher_path, str(i), "best.ckpt"))
        teacher.to(device)
        teacher.eval()
    ...
    for i_epoch in range(1, args.epochs + 1):
        ...
        avg_loss = distill_train(train_loader, model, i_epoch, args, arch_network_type, teacher_lst, test_loader)
