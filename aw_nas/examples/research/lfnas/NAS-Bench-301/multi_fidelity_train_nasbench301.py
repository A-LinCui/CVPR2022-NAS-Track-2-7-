...
from utils import compare_data, prepare, nb301_valid
from dataset import NasBench301Dataset
...
def main(argv):
    ...
    parser.add_argument("--train-pkl", type = str,
            default = os.path.join(PARENT_PATH, "data", "NAS-Bench-301", "nasbench301_train.pkl"), 
            help = "Training Datasets pickle")
    parser.add_argument("--valid-pkl", type = str, 
            default = os.path.join(PARENT_PATH, "data", "NAS-Bench-301", "nasbench301_valid.pkl"), 
    ...
    train_data = NasBench301Dataset(
            train_data, True, args.low_fidelity_type, args.low_fidelity_normalize)
    valid_data = NasBench301Dataset(
            valid_data, False, args.low_fidelity_type, args.low_fidelity_normalize)
    ...
    for i_epoch in range(1, args.epochs + 1):
        ...
        if i_epoch == args.epochs or i_epoch % args.test_every == 0:
            real_corr, patk = nb301_valid(val_loader, model, args)
            logging.info("Valid: Epoch {:3d}: kendall tau {:.4f}; patk {}".\
