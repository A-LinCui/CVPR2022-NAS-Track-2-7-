...
class PointwiseComparator(ArchNetwork, nn.Module):
    ...
    def update_multi_compare(
        self, arch_1, arch_2, better_labels, auxiliary_better_lst, margin = None):
        ...
        if self.compare_loss_type == "binary_cross_entropy":
            ...
            for task in list(auxiliary_better_lst.keys()):
                auxiliary = auxiliary_better_lst[task]
                if task == "market1501_rank" or task == "msmt17_rank":
                    i = 1.
                elif task == "cplfw_rank":
                    i = 0.005
                else:
                    i = 0.001
                pair_loss += i * F.binary_cross_entropy(
                    compare_score, compare_score.new(auxiliary))
    ...
    def cal_triple_loss(self, arch_1, arch_2, arch_3, perf_1, perf_2, perf_3, margin = None):
        s_1 = self.mlp(self.arch_embedder(arch_1)).squeeze()
        s_2 = self.mlp(self.arch_embedder(arch_2)).squeeze()
        s_3 = self.mlp(self.arch_embedder(arch_3)).squeeze()

        pair_loss = sum([
            self._cal_margin_linear_loss(s_1, s_2, perf_1 < perf_2),
            self._cal_margin_linear_loss(s_2, s_3, perf_2 < perf_3),
            self._cal_margin_linear_loss(s_1, s_3, perf_1 < perf_3)
        ])
        ...
        all_s = torch.cat((s_1.unsqueeze(-1), s_2.unsqueeze(-1), s_3.unsqueeze(-1)), -1)
     
        max_s = torch.max(all_s, 1)[0]
        min_s = torch.min(all_s, 1)[0]
        median_s = torch.median(all_s, 1)[0]

        triple_loss = self._cal_margin_linear_loss(max_s - min_s, median_s - min_s, np.zeros_like(perf_1))
        loss = pair_loss + triple_loss
        
        return loss


    def _cal_margin_linear_loss(self, s_1, s_2, better_labels, margin = None):
        better_pm = 2 * s_1.new(np.array(better_labels, dtype = np.float32)) - 1
        zero_ = s_1.new([0.])
        margin = [self.compare_margin] if margin is None else margin
        margin = s_1.new(margin)
...
class MultiTaskPointwiseComparator(PointwiseComparator):
    NAME = "multi_task_pointwise_comparator"

    def __init__(self, search_space, 
            auxiliary_head_num: int, auxiliary_loss_coeff: float = 1.0,
        ...
        # [optional] arch reconstruction loss (arch_decoder_type/cfg)
        super(MultiTaskPointwiseComparator, self).__init__(
        ...
        self.auxiliary_loss_coeff = auxiliary_loss_coeff
        
        dim = self.embedding_dim = self.arch_embedder.out_dim
        self.auxiliary_mlps = nn.ModuleList(self.construct_mlp(dim, mlp_hiddens, mlp_dropout) \
    ...
    def mtl_update_predict(self, archs, labels, auxiliary_datas):
        r"""
        Pretrain the predictor on the one-shot supernet in a multi-task manner with regression loss.

        `self.mlp` is used to calculate scores in the oneshot supernet.
        And `self.auxiliary_mlps` is used to predict the auxiliary objectives (e.g., latency).
        """
        mse_loss = self.cal_predict_loss(archs, labels)
        auxiliary_loss = sum([self._cal_predict_loss(*data, mlp) \
                for (data, mlp) in zip(auxiliary_datas, self.auxiliary_mlps)])
        loss = mse_loss + self.auxiliary_loss_coeff * auxiliary_loss
        return self.update_step(loss)

    def mtl_update_compare(self, arch_1, arch_2, better_labels, auxiliary_datas, margin = None):
        r"""
        Pretrain the predictor on the one-shot supernet in a multi-task manner with ranking loss.

        `self.mlp` is used to calculate scores in the oneshot supernet.
        And `self.auxiliary_mlps` is used to predict the auxiliary objectives (e.g., latency).
        """
        pair_loss = self.cal_compare_loss(arch_1, arch_2, better_labels, margin)
        auxiliary_loss = sum([self._cal_compare_loss(*data, mlp, margin) \
                for (data, mlp) in zip(auxiliary_datas, self.auxiliary_mlps)])
        loss = pair_loss + self.auxiliary_loss_coeff * auxiliary_loss
        return self.update_step(loss)

    def mtl_predict(self, arch, sigmoid = True, tanh = False):
        arch_embedding = self.arch_embedder(arch)
        score_lst = [
            self._single_predict(arch_embedding, mlp, sigmoid, tanh)\
            for mlp in [self.mlp] + list(self.auxiliary_mlps)
        ]
        return score_lst

    def _single_predict(self, arch_embedding, mlp, sigmoid = True, tanh = False):
        score = mlp(arch_embedding).squeeze(-1)
        if sigmoid:
            score = torch.sigmoid(score)
        elif tanh:
            score = torch.tanh(score)
...
class DynamicPointwiseComparator(PointwiseComparator):
    """
    Compatible to NN regression-based predictor of architecture performance.
    """
    NAME = "dynamic_pointwise_comparator"


    def _cal_compare_loss(self, arch_1, arch_2, better_labels, mlp, margin = None):
        arch_1_embedding = [self.arch_embedder(arch) for arch in arch_1]
        arch_2_embedding = [self.arch_embedder(arch) for arch in arch_2]
        s_1 = torch.tensor([mlp(arch_emb).squeeze() for arch_emb in arch_1_embedding])
        s_2 = torch.tensor([mlp(arch_emb).squeeze() for arch_emb in arch_2_embedding])
